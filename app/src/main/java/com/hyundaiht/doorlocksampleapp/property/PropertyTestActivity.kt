...
@Composable
fun PropertyState(states: States, isText: MutableState<Boolean>) {
    val isTextUi by remember { isText }
    ...
    Column(
        ...
        when (states.configuration) {
            is Configuration.MultiOfArray -> {
                PropertyState_MultiOfArray(states.configuration, isTextUi)
            }

            is Configuration.OneOfArray -> {
                PropertyState_OneOfArray(states.configuration, isTextUi)
            }

            is Configuration.OneOfRange -> {
                PropertyState_OneOfRange(states.configuration, isTextUi)
            }

            is Configuration.SingleValue -> {
                PropertyState_SingleValue(states.configuration, isTextUi)
            }

            is Configuration.RecordSet -> {
                PropertyState_RecordSet(states.configuration, isTextUi)
            }
        }
    }
}
...
fun PropertyState_SingleValue(configuration: Configuration.SingleValue, isText: Boolean) {
    ...
    if (isText) {
        ...
        LimitedTypedTextField(
            ...
            Toast.makeText(context, "입력 데이터 : $it", Toast.LENGTH_SHORT).show()
        }
    }
}
...
fun PropertyState_OneOfArray(
    configuration: Configuration.OneOfArray,
    isText: Boolean,
...
fun PropertyState_MultiOfArray(
    configuration: Configuration.MultiOfArray,
    isText: Boolean,
    scope: CoroutineScope = CoroutineScope(Dispatchers.IO)
) {
    if (isText) {
